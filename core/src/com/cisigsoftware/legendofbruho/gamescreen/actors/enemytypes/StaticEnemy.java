...
public class StaticEnemy extends Enemy {
  ...
  private static final String TAG = StaticEnemy.class.getSimpleName();
  ...
  public StaticEnemy(Level level, float x, float y) {
    ...
    setDamage(DAMAGE);
    setLevel(level);
    setState(State.ATTACKING); // Static enemy doesn't have an idle state
    
    Gdx.app.log(TAG, String.format("Created StaticEnemy with HP=%f\tDamage=%f", getHp(), getDamage()));
  }
  ...
  public void act(float delta) {
    ...
    if (!level.isComplete()) {
      ...
      if (!target.isDying() && this.collidesBeside(target)) {
        setAttacked(true);
        target.hurt(damage);
